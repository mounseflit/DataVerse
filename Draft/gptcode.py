...
def preprocess_data(academic_df):
    ...
    academic_df.ffill(inplace=True)  # Replaces deprecated method='ffill'

    # One-hot encode categorical features
    categorical_features = ['SchoolDepartment', 'CourseTitle', 'RequiredSkill', 'GainedSkill']
    encoder = OneHotEncoder(sparse_output=False, handle_unknown='ignore')  # Updated `sparse` to `sparse_output`
...
def train_dbscan(features):
    ...
    return DBSCAN(eps=best_eps, min_samples=5).fit_predict(features)
...
def evaluate_models(features, models):
    ...
    for name, (model, clusters) in models.items():
        if len(set(clusters)) > 1:  # Skip models that fail to create multiple clusters
...
def main():
    ...
    dbscan_clusters = train_dbscan(reduced_features)

    # Evaluate models
    models = {
        "KMeans": (kmeans, kmeans_clusters),
        "DBSCAN": (None, dbscan_clusters)
    }
